...
#End Region
    ...
    'Changed:   
    '--------------------------------------------------------------------------------'
    Public Function glIsTab_id(ByVal lTab_id As Integer) As Integer

        Dim rstsEntityTabAccessTsUsers As Recordset

        rstsEntityTabAccessTsUsers = DataMethodsClientInfo.gRsGetTable(oClientInfo, "tsEntityTabAccessTsUsers", _
            , "etu_tab_id='" & DataTools.SQLString(gsCStr(lTab_id)) & "' And etu_usr_id=" & lUsr_id)

        If rstsEntityTabAccessTsUsers.EOF Then
            Return 2
        Else
            If rstsEntityTabAccessTsUsers("etu_explicit_access").bValue = True Then
                Return 1
            Else
                Return 0
            End If
    ...
    'Changed:   
    '--------------------------------------------------------------------------------'
    Protected Function msGetGroupAccess(ByVal lTab_id As Integer) As String


        If goGetEnvironment(Application).goEntityTabSec.gbHasAnyUserGroupAccessFromDB(oClientInfo, _
            lTab_id, lUsr_id) Then
            Return "Zugriff"
        Else
            Return "kein Zugriff"
        End If
    ...
    'Changed:   
    '--------------------------------------------------------------------------------'
    Protected Function msGetSelectCombo(ByVal lTab_id As Integer) As String
        Dim oSelect As New ComboBox
        oSelect.gAddEntry("2", "wie Gruppe")
        oSelect.gAddEntry("1", "Zugriff")
        oSelect.gAddEntry("0", "kein Zugriff")

        oSelect.sSelectedValue = gsCStr(glIsTab_id(lTab_id))
