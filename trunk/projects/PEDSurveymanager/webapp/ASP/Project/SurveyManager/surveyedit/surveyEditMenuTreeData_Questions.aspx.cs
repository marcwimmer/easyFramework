...
namespace easyFramework.Project.SurveyManager
{
	...
	public class surveyEditMenuTreeData_Questions : efDataPage
	{
		...
		private const string efsFieldList = "qst_id, N'' as qst_parent, " + 
				"qst_text, " +
				"0 as mnu_modalwindow, N'treeview_question' as mnu_icon_normal, N'treeview_question' as mnu_icon_opened, "+
				"N'0' as mnu_isFolder ";
		...
		public override string sGetData(ClientInfo oClientInfo, XmlDocument oRequest)
		{
			...
			string sSvy_id = oRequest.sGetValue("svy_id");
			if (sSvy_id == "")
				return "svy-id is missing for the question-treeview!";

			//-----------Meneintrge---------
			sQry = "SELECT " + efsFieldList + " FROM tdQuestions WHERE qst_svy_id=$1 ORDER BY qst_index";
			sQry = sQry.Replace("$1", sSvy_id);
		...
		//Changed:
		//--------------------------------------------------------------------------------'
		private void mHelperBuildString (ClientInfo oClientInfo, Recordset rsCurrentLine, FastString sResultString, ref int Counter)
		{
			...
			//qst-id:
			sResultString.Append(rsCurrentLine["qst_id"].sValue + "|");
		
			//mnu-text:
			string sDescription = "$0. $1";
			sDescription = sDescription.Replace("$0", Convert.ToString(Counter)); Counter++;
			sDescription = sDescription.Replace("$1", rsCurrentLine["qst_text"].sValue);

			//falls Html-Formatierungen vorhanden sind, diese entfernen:
			sDescription = efHigherFunctions.gsHtml2Text(sDescription);
			...
			//mnu-command:
			string sCommand = "";
			sCommand = "mLoadQuestion('" + rsCurrentLine["qst_id"].sValue + "');";
