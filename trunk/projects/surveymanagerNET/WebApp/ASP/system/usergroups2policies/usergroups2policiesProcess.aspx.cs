...
namespace easyFramework.Project.Default
{
	...
	public class usergroups2policiesProcess : efProcessPage
	{
		...
		public override string sGetData(ClientInfo oClientInfo, XmlDocument oRequest)
		{
			...
			try
			{
				...
				DataMethodsClientInfo.gDeleteTable(oClientInfo, "tsPoliciesTsUserGroups", "pol_grp_grp_id=" + lGrp_id);
				...
				for (int i = 0; i <= nlGroups.lCount - 1; i++)
				{
					...
					if (nlGroups[i].sText != "0")
					{
						...
						DataMethodsClientInfo.gInsertTable(oClientInfo, "tsPoliciesTsUserGroups", "pol_grp_pol_id, pol_grp_grp_id, pol_grp_inserted, pol_grp_insertedBy", "'" + DataTools.SQLString(sPol_id) + "', " + easyFramework.Sys.ToolLib.DataConversion.gsCStr(lGrp_id) + "," + "getdate(), " + "'" + oClientInfo.gsGetUsername() + "'");
